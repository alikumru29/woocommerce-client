...
export class ProductsService {
    ...
    async list(query = {}, options) {
        ...
        const response = await this.client.requestRaw("GET", "/products", {
    ...
    iterate(query = {}, options) {
        ...
        return paginate((page) => this.list({ ...query, page, per_page: perPage }, options).then((r) => r));
    }
    /** Tek ürün getirir. */
    async get(id, options) {
        const body = await this.client.request("GET", `/products/${id}`, options);
        return parseOne(ProductSchema, body, `/products/${id}`);
    }
    /** Yeni ürün oluşturur. */
    async create(input, options) {
        const body = await this.client.request("POST", "/products", {
            body: input,
            ...options,
        });
    ...
    async update(id, input, options) {
        const body = await this.client.request("PUT", `/products/${id}`, {
            body: input,
            ...options,
        });
        return parseOne(ProductSchema, body, `/products/${id}`);
    ...
    async delete(id, opts = {}, options) {
        const body = await this.client.request("DELETE", `/products/${id}`, {
    ...
    async listVariations(productId, query = {}, options) {
        const page = query.page ?? 1;
        const perPage = query.per_page ?? 10;
        const response = await this.client.requestRaw("GET", `/products/${productId}/variations`, { query: { ...query, page, per_page: perPage }, ...options });
        return {
            data: parseArray(ProductVariationSchema, response.body, "variations"),
            meta: extractPaginationMeta(response, page, perPage),
        };
    }
    async getVariation(productId, variationId, options) {
        const body = await this.client.request("GET", `/products/${productId}/variations/${variationId}`, options);
    ...
    async listCategories(query = {}, options) {
        const page = query.page ?? 1;
        const perPage = query.per_page ?? 10;
        const response = await this.client.requestRaw("GET", "/products/categories", {
            query: { ...query, page, per_page: perPage },
            ...options,
        });
        return {
    ...
    async listTags(query = {}, options) {
        const page = query.page ?? 1;
        const perPage = query.per_page ?? 10;
        const response = await this.client.requestRaw("GET", "/products/tags", {
            query: { ...query, page, per_page: perPage },
            ...options,
        });
        return {
    ...
    async listReviews(query = {}, options) {
        const page = query.page ?? 1;
        const perPage = query.per_page ?? 10;
        const response = await this.client.requestRaw("GET", "/products/reviews", {
            query: { ...query, page, per_page: perPage },
            ...options,
        });
        return {
