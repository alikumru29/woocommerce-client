...
export class OrdersService {
    ...
    async list(query = {}, options) {
        ...
        const response = await this.client.requestRaw("GET", "/orders", {
    ...
    async get(id, options) {
        const body = await this.client.request("GET", `/orders/${id}`, options);
        return parseOne(OrderSchema, body, `/orders/${id}`);
    }
    async create(input, options) {
        const body = await this.client.request("POST", "/orders", {
            body: input,
            ...options,
        });
    ...
    async update(id, input, options) {
        const body = await this.client.request("PUT", `/orders/${id}`, {
            body: input,
            ...options,
        });
        return parseOne(OrderSchema, body, `/orders/${id}`);
    ...
    async delete(id, opts = {}, options) {
        const body = await this.client.request("DELETE", `/orders/${id}`, {
    ...
    // --- Order Notes ---
    async listNotes(orderId, query = {}, options) {
        const body = await this.client.request("GET", `/orders/${orderId}/notes`, {
            query,
            ...options,
        });
        return parseArray(OrderNoteSchema, body, `/orders/${orderId}/notes`);
    }
    async addNote(orderId, input, options) {
        const body = await this.client.request("POST", `/orders/${orderId}/notes`, {
            body: input,
            ...options,
        });
        return parseOne(OrderNoteSchema, body, `/orders/${orderId}/notes`);
    }
    async deleteNote(orderId, noteId, options) {
        const body = await this.client.request("DELETE", `/orders/${orderId}/notes/${noteId}`, { query: { force: true }, ...options });
